#!/usr/bin/python3      # set the interpreter
import pgzrun           # import pgzero module
...
def draw():
    screen.clear()
    # draw the track
    screen.blit('track_ny', (0,0))
    # draw distance markers and finish line
    displayAt('finishline', 1, 230)
    displayAt('cloud', 1, 10)
    ...
    screen.draw.text('Time (s): ' + str(round(sprinter.finishTime - sprinter.startTime, 2)), (200, 20), fontsize=32, color="white")
    screen.draw.text('Hastighet (km/h): ' + str(round(sprinter.speed * 1000)), (400, 20), fontsize=32, color="white")
    screen.draw.text('Växel : ' + str(round(sprinter.gear)),(20,50), fontsize=32, color="white")
